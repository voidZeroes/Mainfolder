...
public class SpinArm : MonoBehaviour
{
   ...
   public Vector2 GetGenPos()
        ...
        private bool CalcLRPad()//左右反転するやつ
    {
        ...
        //BulletGenを利用してアームキャノンの方向を確認
        if (this.transform.position.x < check.transform.position.x)//Checkのｘが自分より大きいなら右
        {
                flg = true;
        }

        if (this.transform.position.x > check.transform.position.x)//逆なら左
        {
                flg = false;
        }
